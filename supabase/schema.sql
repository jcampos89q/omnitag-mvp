-- Habilitar extensiÃ³n pgcrypto para UUIDs
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- 1. Tabla de Usuarios (ExtensiÃ³n de auth.users)
...
CREATE TABLE public.vcards (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE, -- DueÃ±o de la tarjeta
  ...
  company_name TEXT,
  bio TEXT,
  contact_info JSONB DEFAULT '{}'::jsonb, -- TelÃ©fono, email, redes sociales
  theme JSONB DEFAULT '{"color": "#000000"}'::jsonb, -- Opciones de personalizaciÃ³n
...
CREATE TABLE public.devices (
  ...
  workspace_id UUID REFERENCES public.workspaces(id) ON DELETE SET NULL, -- Empresa dueÃ±a
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE, -- Usuario que activÃ³ el dispositivo
  ...
  redirect_url TEXT, -- Si es Tap-to-Rate, aquÃ­ va la URL (Google Reviews Place ID)
  is_active BOOLEAN DEFAULT false, -- Inicialmente falso hasta que el cliente lo active escaneando
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- 6. Tabla de AnalÃ­tica (Tracking simple)
